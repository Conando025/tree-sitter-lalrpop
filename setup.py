...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_lalrpop", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_lalrpop": ["*.pyi", "py.typed"],
        "tree_sitter_lalrpop.queries": ["*.scm"],
    },
    ext_package="tree_sitter_lalrpop",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_lalrpop/binding.c",
                "src/parser.c",
            ],
            define_macros=[
